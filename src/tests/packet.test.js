...
describe('signing packet', () => {
  test('active packet documents ride in the same agreement, in order, behind the offer', async () => {
    ...
    const outComment = backend.updates.find((u) => u.body.includes('out for signature'));
    expect(outComment.body).toContain('Hire packet is out for signature');
  ...
  test('empty catalog keeps the plain single-document offer send', async () => {
    ...
    const outComment = backend.updates.find((u) => u.body.includes('out for signature'));
    expect(outComment.body).toContain('Offer is out for signature');
