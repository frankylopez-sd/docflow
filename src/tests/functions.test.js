...
describe('Azure Function entrypoints', () => {
  ...
  test('sendForSign entry fails clearly when no PDF link exists to hydrate', async () => {
    ...
    expect(backend.rows[555].written.status).toEqual({ label: config.load().monday.statusLabels.signFailed });
    expect(backend.rows[555].written[config.load().monday.columns.offerStatus]).toEqual({ label: config.load().monday.offerLabels.failed });
  });
...
describe('signPoller fallback', () => {
  beforeEach(() => {
    // Seed the exact label the poller scans for (config vocabulary, never hardcoded)
    backend.rows[555].written.status = { label: config.load().monday.statusLabels.outForSignature };
